...
class LogisticRegression:
    ...
    def gradient_descent(data,labels,current_initial_theta,max_iterations):
        ...
        optimized_theta = current_initial_theta.copy()
        for _ in range(max_iterations):
            gradients = LogisticRegression.gradient_step(data,labels,optimized_theta)
            optimized_theta = optimized_theta - 0.3 * gradients[:,np.newaxis]
            cost_history.append(LogisticRegression.cost_function(data,labels,optimized_theta))
        return optimized_theta,cost_history

    @staticmethod
    def cost_function(data,labels,theat):
        num_examples = data.shape[0]
        predictions = LogisticRegression.hypothesis(data,theat) # 只是线性回归函数加了一个sigmoid
    ...
    def hypothesis(data,theat):
        ...
        predictions = sigmoid(np.dot(data,theat)) # sigmoid函数将 模型结果缩放到0-1
